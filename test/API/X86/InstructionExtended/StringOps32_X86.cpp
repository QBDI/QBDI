...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-CMPSL") {
  ...
  state->esi = (QBDI::rword)&bufA[0];
  state->edi = (QBDI::rword)&bufB[0];
  state->eflags &= ~(1 << 10);
  ...
  CHECK(ran);
  CHECK(seenCount == 1);
  CHECK(state->esi == (QBDI::rword)&bufA[1]);
  CHECK(state->edi == (QBDI::rword)&bufB[1]);
}

TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-LODSL") {
  ...
  CHECK(ran);
  CHECK(seen);
  CHECK((state->eax & 0xffffffff) == 0x11223344);
  CHECK(state->esi == (QBDI::rword)&buf[1]);
}

TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-MOVSL") {
...
TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-SCASL") {
  ...
  CHECK(ran);
  CHECK(seen);
  CHECK(state->edi == (QBDI::rword)&buf[1]);
}

TEST_CASE_METHOD(APITest, "InstructionExtendedTest_X86-STOSL") {
