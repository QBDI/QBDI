...
Assembly::Assembly(llvm::MCContext &MCTX,
                   std::unique_ptr<llvm::MCAsmBackend> MAB,
                   llvm::MCInstrInfo &MCII, const llvm::Target *target,
                   llvm::MCSubtargetInfo &MSTI, Options options)
    : target(target), MCII(MCII), MRI(*MCTX.getRegisterInfo()),
      MAI(*MCTX.getAsmInfo()), MSTI(MSTI), options(options) {
  ...
  if (((opts ^ options) & Options::OPT_ATT_SYNTAX) != 0) {
    asmPrinter =
        std::unique_ptr<llvm::MCInstPrinter>(target->createMCInstPrinter(
            ...
            ((opts & Options::OPT_ATT_SYNTAX) == 0) ? 1 : 0, MAI, MCII, MRI));
    asmPrinter->setPrintImmHex(true);
    asmPrinter->setPrintImmHex(llvm::HexStyle::C);
  }
#endif
...
Assembly::getInstruction(llvm::MCInst &instr, uint64_t &size,
...
void Assembly::writeInstruction(const llvm::MCInst inst,
...
std::string Assembly::showInst(const llvm::MCInst &inst,
                               uint64_t address) const {
  std::string out;
