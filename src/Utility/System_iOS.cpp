...
bool isRWXSupported() {
    ...
    std::call_once(rwxflag, [](){
        ...
        std::error_code ec = llvm::sys::Memory::protectMappedMemory(block, PF::MF_READ | PF::MF_WRITE | PF::MF_EXEC);
        success = ec.value() == 0;
        ...
        while (success) {
            ...
            if (info.is_submap) {
                ...
                vm_prot_t requested_prot = VM_PROT_READ | VM_PROT_WRITE | VM_PROT_EXECUTE;
...
llvm::sys::MemoryBlock allocateMappedMemory(size_t numBytes,
    ...
    mach_vm_address_t page = 0; // careful here to use correct type as size can be different
    bool setPerms = true;
...
const std::string getHostCPUName() {
    ...
    if (hostInfo.cpu_type == CPU_TYPE_ARM) {
        switch(hostInfo.cpu_subtype) {
            case CPU_SUBTYPE_ARM_V4T:    return "arm710t";
            case CPU_SUBTYPE_ARM_V6:     return "arm1136j-s";
            case CPU_SUBTYPE_ARM_V5TEJ:  return "arm9e";
            case CPU_SUBTYPE_ARM_XSCALE: return "xscale";
            case CPU_SUBTYPE_ARM_V7:     return "cortex-a8";
            case CPU_SUBTYPE_ARM_V7F:    return "cortex-a9";
            case CPU_SUBTYPE_ARM_V7S:    return "swift";
