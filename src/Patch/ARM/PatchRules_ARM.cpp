...
RelocatableInst::SharedPtrVec getExecBlockPrologue(Options opts) {
  ...
  // Move sp at the beginning of the data block to solve memory addressing range
  // problems This instruction needs to be EXACTLY HERE for relative addressing
  // alignement reasons
  prologue.push_back(Adr(Reg(REG_SP), 4080));
  append(prologue,
         SaveReg(Reg(REG_BP), Offset(offsetof(Context, hostState.fp))));
  // Restore FPR
  for (unsigned int i = 0; i < QBDI_NUM_FPR; i++)
    prologue.push_back(
        Vldrs(llvm::ARM::S0 + i, Reg(REG_SP),
              offsetof(Context, fprState.s) + i * sizeof(float)));
  // Restore CPSR
  prologue.push_back(
      Ldr(Reg(0), Reg(REG_SP), offsetof(Context, gprState.cpsr)));
...
RelocatableInst::SharedPtrVec getExecBlockEpilogue(Options opts) {
  ...
  for (unsigned int i = 0; i < QBDI_NUM_FPR; i++)
    epilogue.push_back(
        Vstrs(llvm::ARM::S0 + i, Reg(REG_SP),
              offsetof(Context, fprState.s) + i * sizeof(float)));
  // Save CPSR
  ...
  append(epilogue,
         LoadReg(Reg(REG_BP), Offset(offsetof(Context, hostState.fp))));
  append(epilogue,
...
PatchRule::SharedPtrVec getDefaultPatchRules() {
   ...
   */
  rules.push_back(PatchRule(Or({OpIs(llvm::ARM::BX), OpIs(llvm::ARM::BX_pred)}),
   ...
   *      SimulateLink(Temp(0))
   */
  rules.push_back(PatchRule(Or({OpIs(llvm::ARM::BL), OpIs(llvm::ARM::BL_pred),
                                OpIs(llvm::ARM::BLXi)}),
   ...
   */
  rules.push_back(
      PatchRule(Or({OpIs(llvm::ARM::MOVPCLR), OpIs(llvm::ARM::BX_RET)}),
   ...
   */
  rules.push_back(
      PatchRule(OpIs(llvm::ARM::Bcc),
                {GetPCOffset(Temp(0), Operand(0)),
                 ModifyInstruction({SetOperand(Operand(0), Constant(0))}),
                 GetPCOffset(Temp(0), Constant(-4)),
                 WriteTemp(Temp(0), Offset(Reg(REG_PC)))}));
   ...
   *      SimulateRet(Temp(0))
   */
  rules.push_back(PatchRule(And({
                                OpIs(llvm::ARM::LDMIA_UPD),
                                UseReg(Reg(REG_PC)),
                            }),
                            {ModifyInstruction({RemoveOperand(Reg(REG_PC))}),
   ...
   */
  rules.push_back(PatchRule(
      RegIs(Operand(0), Reg(REG_PC)),
      {GetPCOffset(Temp(0), Constant(0)), GetPCOffset(Temp(1), Constant(-4)),
       ModifyInstruction({SubstituteWithTemp(Reg(REG_PC), Temp(0)),
