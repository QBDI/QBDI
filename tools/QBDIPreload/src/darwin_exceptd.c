...
static void* exception_server(void* data) {
    ...
    mach_msg_header_t* msg = (mach_msg_header_t*) calloc(1, sizeof(__Request__mach_exception_raise_state_identity_t));
    mach_msg_header_t* reply = (mach_msg_header_t*) calloc(1, sizeof(__Request__mach_exception_raise_state_identity_t));

    while (exceptionHandler->running) {
        rt = mach_msg(msg, MACH_RCV_MSG, 0, sizeof(__Request__mach_exception_raise_state_identity_t), exceptionHandler->port, 200, MACH_PORT_NULL);
        if(rt == MACH_MSG_SUCCESS) {
            mach_exc_server(msg, reply);
            rt = mach_msg(reply, MACH_SEND_MSG, reply->msgh_size, 0, MACH_PORT_NULL, 500, MACH_PORT_NULL);
            if(rt != MACH_MSG_SUCCESS) {
    ...
    kr = task_set_exception_ports(exceptionHandler->target, exceptionHandler->mask, MACH_PORT_NULL, 0, 0);
    if(kr != KERN_SUCCESS) {
...
struct ExceptionHandler* setupExceptionHandler(task_t target, exception_mask_t exceptionMask, exception_handler_func handler) {
    ...
    pthread_attr_t attr;
    kern_return_t kr;
    struct ExceptionHandler* exceptionHandler = calloc(1, sizeof(struct ExceptionHandler));
    ...
    if(REGISTERED_HANDLERS == NULL) {
        ...
        REGISTERED_HANDLERS = (struct ExceptionHandler**) calloc(REGISTERED_HANDLERS_CAPACITY, sizeof(struct ExceptionHandler*));
    }
    
    kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &exceptionHandler->port);
    if(kr != KERN_SUCCESS) {
    ...
    kr = mach_port_insert_right(mach_task_self(), exceptionHandler->port, exceptionHandler->port, MACH_MSG_TYPE_MAKE_SEND);
    if(kr != KERN_SUCCESS) {
    ...
    kr = task_set_exception_ports(target, exceptionHandler->mask, exceptionHandler->port, EXCEPTION_DEFAULT|MACH_EXCEPTION_CODES, THREAD_STATE_NONE);
    if(kr != KERN_SUCCESS) {
    ...
    if(REGISTERED_HANDLERS_COUNT == REGISTERED_HANDLERS_CAPACITY) {
        // Realloc
        struct ExceptionHandler** newlist = (struct ExceptionHandler**) calloc(REGISTERED_HANDLERS_CAPACITY * 2, sizeof(struct ExceptionHandler*));
        // Copy old list
        memcpy(newlist, REGISTERED_HANDLERS, REGISTERED_HANDLERS_COUNT*sizeof(struct ExceptionHandler));
        // Free and replace
