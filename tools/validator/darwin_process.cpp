...
DarwinProcess::DarwinProcess(pid_t process):  pid(process), brk_address(nullptr), brk_value(0),
    ...
    QBDI::qbdipreload_setup_exception_handler(this->task, EXC_MASK_BREAKPOINT | EXC_MASK_CRASH, (void*) &onBreakpoint);
}
...
void DarwinProcess::makeRW(void* address) {
    kern_return_t kr;
    QBDI::rword page = (QBDI::rword) address - ((QBDI::rword) address % this->pageSize);
